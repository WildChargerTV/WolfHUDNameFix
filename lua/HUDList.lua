...
if string.lower(RequiredScript) == "lib/managers/hudmanagerpd2" then
    ...
    HUDListManager.LOOT_SETTINGS = {
        mex = {
            armor = {
                start_disabled = { "blueprint_collected" },
                enable_events = {
                    {
                        source = "special_equipment",
                        events = { "interact" },
                        keys = { "hold_take_vault_blueprint" },
                        reason = "blueprint_collected"
                    }
                },
                disable_events = {}
            }
    ...
    function HUDListManager:_set_show_loot()
        ...
        if HUDListManager.ListOptions.aggregate_loot then
            ...
            for loot_type, members in pairs(all_types) do
                if HUDListManager.ListOptions.show_loot then
                    local settings = HUDListManager.LOOT_SETTINGS
                    local level_id = managers.job:current_level_id()
                    local class = settings[level_id] and settings[level_id][loot_type] and settings[level_id][loot_type].class or settings[loot_type] and settings[loot_type].class or HUDList.LootItem
                    list:register_item(loot_type, class, loot_type, members)
                else
    ...
    HUDList.SpecialPickupItem.MAP = {
        ...
        c4 = 						{ hud_icons	 = { 36, 242, 32, 32 }, 											priority = 1, category = "mission_pickups", ignore = not WolfHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
        weapon_part = 				{ texture = "guis/dlcs/ranc/textures/pd2/equipment_stock", 						priority = 1, category = "mission_pickups", ignore = not WolfHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
    ...
    HUDList.LootItem.MAP = {
        ...
        tea = 			{ text = "wolfhud_hudlist_loot_tea", 		priority = 1 },	-- Tea Service
        dragon = 		{ text = "wolfhud_hudlist_loot_dragon", 	priority = 1 },	-- Dragon Heist
        gnome = 		{ text = "hud_carry_gnome", 				priority = 1 },	-- L4D-Gnome
        ...
        bux =           { text = "wolfhud_hudlist_loot_bux", 		priority = 2, no_separate = true },	-- Piggy Bux (9th Anniversary)
    ...
    function HUDList.LootItem:init(parent, name, id, members)
        ...
        local level_id = managers.job:current_level_id() or ""
        local loot_settings = HUDListManager.LOOT_SETTINGS[level_id] and HUDListManager.LOOT_SETTINGS[level_id][id] or HUDListManager.LOOT_SETTINGS[id]
        if loot_settings ~= nil then
            for _, reason in ipairs(loot_settings.start_disabled) do
                self:set_disabled(reason, true, true)
            end
            for _, data in ipairs(loot_settings.enable_events) do
                table.insert( self._listener_clbks, 
                {
                    name = string.format("HUDList_%s_enable_event_%s_listener", id, data.reason),
                    source = data.source,
                    event = data.events,
                    clbk = callback(self, self, "_enabled_by_event_clbk", data.reason),
                    keys = data.keys,
                    data_only = true
                })
            end
            
            for _, data in ipairs(loot_settings.disable_events) do
                table.insert( self._listener_clbks, 
                {
                    name = string.format("HUDList_%s_enable_event_%s_listener", id, data.reason),
                    source = data.source,
                    event = data.event,
                    clbk = callback(self, self, "_disabled_by_event_clbk", data.reason),
                    keys = data.keys,
                    data_only = true
                })
            end
    ...
    function HUDList.LootItem:_enabled_by_event_clbk(reason, data)
        self:set_disabled(reason, false, false)
        self._active_count_change = {
            duration = 0.5,
            current = 0,
            color = self._change_increase_color,
            invert_progress = false
        }
    end

    function HUDList.LootItem:_disabled_by_event_clbk(reason, data)
        self._active_count_change = {
            duration = 0.5,
            current = 0,
            color = self._change_decrease_color,
            invert_progress = true
        }
        self:set_disabled(reason, true, false)
    end
    ...
    HUDList.TimerItem.DEVICE_TYPES = {
        ...
        piggy_bank      = { class = "PiggyBankTimerItem", 	name = "wolfhud_hudlist_device_piggy_bank"  },
    }
    function HUDList.TimerItem:init(parent, name, data)
        self.STANDARD_COLOR = self.STANDARD_COLOR or HUDListManager.ListOptions.list_color or Color(1, 1, 1, 1)
        self.DISABLED_COLOR = self.DISABLED_COLOR or Color(1, 1, 0, 0)
        self.FLASH_SPEED = 2
    ...
    function HUDList.SecurityTimerItem:init(parent, name, data)
        ...
        self:_set_current_bar(data)
        self:_set_total_bars(data)
    ...
    function HUDList.SecurityTimerItem:_set_current_bar(data)
        if data.current_bar  and self._bars.current ~= data.current_bar then
    ...
    function HUDList.SecurityTimerItem:_set_total_bars(data)
        if data.total_bars and self._bars_total ~= data.total_bars then
            self._bars.total = data.total_bars
            self:_update_bar_text()
        end
    end

    HUDList.PiggyBankTimerItem = HUDList.PiggyBankTimerItem or class(HUDList.SecurityTimerItem)
    function HUDList.PiggyBankTimerItem:init(parent, name, data)
        data.show_distance = false	-- Disabled, we show current and total bars there.
        self.STANDARD_COLOR = self.STANDARD_COLOR or WolfHUD:getColor("orange")
        
        HUDList.PiggyBankTimerItem.super.init(self, parent, name, data)
        ...
        local id = string.format("HUDList_timer_listener_%s", key)
        local events = {
            set_progress = callback(self, self, "_set_progress"),
        }
    ...
    function HUDList.PiggyBankTimerItem:_set_progress(data)
        local current = data.current or 0
        local total = data.total or 0
        local remaining = total - current
        
        if data.current and data.total and self._remaining ~= remaining then
            self._remaining = remaining

            self._time_text:set_text(remaining)
            self._progress_bar:set_ratio(data.progress or 0)
        end
    end

    function HUDList.PiggyBankTimerItem:_update_bar_text()
        self._distance_text:set_text(string.format( "Lvl. %d / %d", self._bars.current or 1, self._bars.total or 3 ))
    end
    ...
    function HUDList.MinionItem:init(parent, name, data)
        ...
        local key = self._name --tostring(self._unit:key())
    ...
    function HUDList.ECMRetriggerItem:init(parent, name, data)
        ...
        local key = self._name --tostring(data.unit:key())
        table.insert(self._listener_clbks, {
    ...
    HUDList.BuffItemBase.MAP = {
        ...
        -- Special Events:
        piggy_bag = {
            hud_icons = { 70, 310, 32, 32 },
            class = "BuffItemBase",
            priority = 0,
            color = HUDList.BuffItemBase.ICON_COLOR.SPECIAL,
        },
    }
